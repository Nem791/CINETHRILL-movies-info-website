...
fetch(`../db.json`)
  ...
  .then((data) => {
      ...
      for (let i = 0; i < datafullList.length; i++) {
        ...
        if (datafullList[i].imdb_id == item) {
          document.getElementById(
            "movie_results"
          ).innerHTML += `<div class="movie_result">
          ...
          let movie_id = document.getElementsByClassName(
            "movie-result-item-button-watch"
          );
          ...
          let movie_id_remove = document.getElementsByClassName(
            "movie-result-item-button-remove"
          );
            ...
            movie_id_remove[i].addEventListener("click", () => {
              if (confirm("Are you sure you want to remove this movie?")) {
                ...
                localStorage.setItem(
                  "moviesInCart",
                  JSON.stringify(tempMovieId)
                );
                let parentDiv = document.getElementById(
                  dataListresult[i].imdb_id
                ).parentNode;
    ...
    if (dataListresult.length == 0) {
      document.getElementById(
        "movie_results"
      ).innerHTML += `<h3> We cannot find any results </h3>`;
    }
  })
  .catch((err) => {
    console.error(err);
