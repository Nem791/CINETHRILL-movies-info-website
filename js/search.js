  ...
  fetch(`../db.json`)
    ...
    .then((data) => {
      ...
      for (let i = 0; i < datafullList.length; i++) {
        if (
          datafullList[i].title.includes(x) ||
          datafullList[i].title.toLowerCase().includes(x) ||
          datafullList[i].title.toUpperCase().includes(x)
        ) {
          ...
          const movie_id = document.getElementsByClassName(
            "movie-result-item-button"
          );
      ...
      if (dataListresult.length == 0) {
        document.getElementById(
          "movie_results"
        ).innerHTML += `<h3 class="list-message"> We cannot find any results </h3>`;
      }
    })
    .catch((err) => {
      console.error(err);
